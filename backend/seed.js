    ...
    try {
        ...
        const coursesByInstructor = coursesData.map((course, index) => {
            ...
            const enrolledStudents = students
                ...
                if (studentIndex === 0 && index < 24)
                    return true;
                if (studentIndex === 1 && index % 3 === 0)
                    return true;
                if (studentIndex === 2 && index % 2 === 0)
                    return true;
                if (studentIndex === 3 && index % 4 !== 0)
                    return true;
                if (studentIndex === 4 && index % 5 === 0)
                    return true;
                if (studentIndex === 5 && index % 3 === 1)
                    return true;
                return false;
            })
        ...
        for (const course of createdCourses) {
            const courseReviews = createdReviews.filter((review) => String(review.course) === String(course._id));
            if (courseReviews.length === 0)
                continue;
            const averageRating = courseReviews.reduce((sum, review) => sum + review.rating, 0) / courseReviews.length;
            course.ratingAndReviews = courseReviews.map((review) => review._id);
            course.averageRating = Math.round(averageRating * 10) / 10;
            course.totalRatings = courseReviews.length;
            await course.save();
        }
        for (const instructor of instructors) {
            const instructorCourses = createdCourses.filter((course) => String(course.instructor) === String(instructor._id));
            instructor.courses = instructorCourses.map((course) => course._id);
            instructor.totalRevenue = instructorCourses.reduce((sum, course) => sum + (course.totalRevenue || 0), 0);
            await instructor.save();
        }
        const studentToCoursesMap = new Map();
